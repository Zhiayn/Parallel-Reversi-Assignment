...
{
  int i, move_score; //create variables for the move's index and move score
  int pid = fork (); // forks the processes, the integer is to record if the process is a child or not
  if (pid == 0) // if the process is a child
    {
      for (i=0; i < noOfMoves; i++) //search every move until the number of moves given is reached
	{
	  multiprocessor_wait (processorAvailable); //waits until a core is available
	  if (fork () == 0) //tests if the process is another child, for those children are to search the alphabeta tree for moves as shown below
	    {
	      move_score = alphaBeta (l[i], c, u, noPlies, o, minscore, maxscore); //the alpha-beta algorithm calculates the move score for the given move. The move score is compared to the current best score later.
	      mailbox_send (barrier, move_score, i, positionsExplored); // records the move's score, number and the positions explored to the mailbox for the parent to read
	      multiprocessor_signal (processorAvailable); // signals that a core is now free.
	      exit (0);
	    }
	}
      exit(0);
    ...
    {
      int i, move_score, move_index, positions_explored; //these are the variables that will record the data recieved by the mailbox, via pointing towards the reference

      for (i=0; i < noOfMoves; i++) //do this until the number of given moves has been reached
	{
	  printf ("Parent is waiting for the result\n"); //message for the user playing the game, to keep track of the program's logic
	  mailbox_rec (barrier, &move_score, &move_index, &positions_explored); //opens and recieves the data in the mailbox sent by the children
	  printf ("...result recieved! Move %d has a score of %d after searching %d positions\n", //message for the user playing the game, displays the information sent by the children
		  move_index, move_score, positions_explored);
	  *totalExplored += positions_explored; //updates the total amount of positions by adding the number of positions the child explored
	  if (move_score > best)
	    {							//if the move recieved is better then the current best move
	      best = move_score;		//replace the current best score with the new one
	      *move = l[move_index];	//and record which index the move is in, so it knows which move to play
	    }
	}
    }
  return best;
